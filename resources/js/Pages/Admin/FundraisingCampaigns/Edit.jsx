...
export default function Edit({ campaign }) {
    ...
    const { data, setData, post, processing, errors, reset } = useForm({
        ...
        payment_methods: campaign.payment_methods || ['bank_transfer', 'mobile_money'],
    ...
    const handleSubmit = (e) => {
        ...
        // For updates with file uploads, we use router.post with FormData
        ...
        Object.keys(data).forEach(key => {
            if (data[key] !== null && data[key] !== undefined) {
                if (key === 'payment_methods') {
                    // Handle array data
                    data[key].forEach((method, index) => {
                        formData.append(`payment_methods[${index}]`, method);
                    });
                } else if (key === 'image_file' || key === 'video_file') {
                    // Handle file uploads
                    if (data[key]) {
                        formData.append(key, data[key]);
                    }
                } else if (key === 'is_featured') {
                    // Handle boolean values - convert to string that Laravel can understand
        ...
        router.post(route('admin.fundraising-campaigns.update', campaign.id), formData, {
            onSuccess: () => {
                success('Success', 'Campaign updated successfully');
            },
            onError: (errors) => {
                console.log('Update errors:', errors);
                error('Error', 'Failed to update campaign. Please check the form for errors.');
            }
        });
    };
        ...
        <SidebarLayout>
            ...
            <div className="mb-6">
                <div className="flex items-center mb-4">
                    <Link href={route('admin.fundraising-campaigns.show', campaign.id)}>
                        <SecondaryButton>
            ...
            <div className="bg-white dark:bg-gray-800 shadow rounded-lg">
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                        ...
                        {campaign.image_path && !data.image_file && (
                            <div className="mb-6">
                        ...
                        {campaign.video_path && !data.video_file && (
                            <div className="mb-6">
                                <InputLabel value="Current Video" />
                                <div className="mt-2 p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    <video
                                        src={campaign.video_url_full}
                                        controls
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <FileInput
                                    id="image_file"
                                    name="image_file"
                                    label={campaign.image_path ? "Upload New Campaign Image" : "Upload Campaign Image"}
                                    accept="image/*"
                                    maxSize={5}
                            ...
                            <div>
                                <FileInput
                                    id="video_file"
                                    name="video_file"
                                    label={campaign.video_path ? "Upload New Campaign Video" : "Upload Campaign Video (Optional)"}
                                    accept="video/*"
                                    maxSize={50}
